...
class FeedforwardGateI(nn.Module):
    ...
    def forward(self, *prev_features):
        features = torch.cat(prev_features, 1)
        if features.size(2)*features.size(3) != 1:
        ...
        if features.size(2) * features.size(3) != 1:
            features = self.avg_layer(features)

        #print('Linear layer input : {}'.format(features.shape))
        #features = self.linear_layer(features).squeeze()
        features = self.linear_layer(features)
        #print('Linear layer output : {}'.format(features.shape))
        softmax = self.prob_layer(features)
        #print('gate-probability output : {}'.format(softmax))
        #logprob = self.logprob(features)

        # discretize output in forward pass.
        # use softmax gradients in backward pass
        features = (softmax[:, 1] > 0.5).float().detach() - \
            softmax[:, 1].detach() + softmax[:, 1]
        #print('mask output : {}'.format(features.shape))
...
class _DenseLayer(nn.Module):
    ...
    def forward(self, *prev_features):  # I want to know what the *prev_features is
        ...
        #if se_features.size(0) != 1:
        se_features = self.norm2(se_features)
        new_features = self.conv2(self.relu2(se_features))
...
class _DenseBlock(nn.Module):
    ...
    def forward(self, init_features):
        ...
        for i in range(self.num_layers):
            ...
            #print('Output of each denselayer is : {}'.format(new_features.shape))
            # mask = mask.squeeze()
            # mask = mask.expand(64, 768)
            #print('Output of each Dense layer : {}'.format(new_features.shape))
            #print('Output of gprob : {}'.format(gprob))
            """
            gate_features = getattr(self, 'gatelayer{}'.format(i + 1))(*features)
            #mask = gate_features[0]
            new_features = getattr(self, 'denselayer{}'.format(i + 1))(*features)
            mask = torch.ones([1, 1, 1, 1], dtype=torch.float)
...
class Skip_DGNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16, 16), compression=0.5,
        ...
        # Final BatchNom
        self.add_module('norm_final', nn.BatchNorm2d(num_features))
        # Initialization
...
class DGModel(nn.Module):
    def __init__(self):
        ...
        self.features = Skip_DGNet(growth_rate=12, block_config=(16, 16, 16, 16), compression=0.5,
                                   num_init_features=24, bn_size=4, drop_rate=0, small_inputs=False, efficient=True)
